  ...
  try {
    ...
    for (let i = 0; i < mediaFiles.length; i++) {
      ...
      if (uploadError) {
        ...
        return NextResponse.json(
          { success: false, message: `Erreur upload fichier ${file.name}: ${uploadError.message}` },
          { status: 500 }
        )
      }
    ...
    if (projectError) {
      ...
      return NextResponse.json(
        { success: false, message: `Erreur création projet: ${projectError.message}` },
        { status: 500 }
      )
    }
    ...
    if (uploadedMedias.length > 0) {
      const mediaInserts = uploadedMedias.map((media) => ({
      ...
      if (mediaError) {
        ...
        return NextResponse.json(
          { success: false, message: `Erreur insertion médias: ${mediaError.message}` },
          { status: 500 }
        )
      }
    }
